(function() {
  ...
  callWithJQuery(function($) {
    ...
    Utilities
     */
    var PivotData, addSeparators, aggregatorTemplates, aggregators, dayNamesEn, derivers, getSort, locales, mthNamesEn, naturalSort, numberFormat, pivotTableRenderer, rd, renderers, rx, rz, sortAs, usFmt, usFmtInt, usFmtPct, zeroPad,sumColCount=1;
    ...
    aggregatorTemplates = {
      ...
      sum: function(formatter) {
        ...
        return function(arg) {
          ...
          return function(data, rowKey, colKey) {
            return {
              sum: new Array(attr.length).fill(0),
              push: function(record) {
                	for(var index in attr){
                		if (!isNaN(parseFloat(record[attr[index]]))) {
                			this.sum[index] += parseFloat(record[attr[index]])
                		}
                	}
                  return true;
              ...
              format: formatter,
              numInputs: attr[0] != null ? 0 : sumColCount
            };
          };
        };
      },
        ...
        if ($.isFunction(input)) {
          ...
          if ($.isArray(input[0])) {
            ...
            for (i in input) {
              ...
              results.push(addRecord(record,i));
            }
            return results;
          } else {
            results1 = [];
    ...
    pivotTableRenderer = function(pivotData, opts) {
      var aggregator, c, colAttrs, colKey, colKeys, defaults, getClickHandler, i, j, r, result, rowAttrs, rowKey, rowKeys, spanSize, tbody, td, th, thead, totalAggregator, tr, txt, val, x,valAttrs;
      defaults = {
        table: {
          clickCallback: null
        },
      ...
      valAttrs = pivotData.valAttrs.length==0 ? [""] : pivotData.valAttrs;
      ...
      if (rowAttrs.length !== 0) {
        ...
        for(var valIndex in valAttrs){
        	th = document.createElement("th");
            if (colAttrs.length === 0) {
              th.className = "pvtTotalLabel";
              var dataString = valAttrs[valIndex]===""?"":"("+valAttrs[valIndex]+")";
              th.innerHTML = opts.localeStrings.totals + dataString;
            }
            tr.appendChild(th);
        }
      ...
      for (i in rowKeys) {
        ...
        for(var valIndex in valAttrs){
	        ...
	        val = totalAggregator.value(valIndex);
	        td = document.createElement("td");
    ...
    $.fn.pivotUI = function(input, inputOpts, overwrite, locale) {
      ...
      try {
        ...
        rendererControl = $("<div>").addClass('pvtRendererControl');
        ...
        unused = $("<div>").addClass('pvtAxisContainer pvtUnused divColList');
        ...
        fn1 = function(attr) {
          ...
          triangleLink = $("<span>").addClass('pvtTriangle pvtTrCenter').html(" &#x25BE;").bind("click", function(e) {
            var left, ref2, top;
          ...
          attrChild = $("<span>").addClass('pvtAttr').text(attr).data("attrName", attr);
          attrElem = $("<li>").addClass("axis_" + i).append($("<span>").attr("style","position:relative").html(attrChild).append(triangleLink));
          if (hasExcludedItem) {
            attrElem.addClass('pvtFilteredAttribute');
          }
        ...
        rendererControl.append(aggregator);
        
        $("<td colspan=2>").addClass('pvtVals').appendTo(tr1).prepend(rendererControl);
        $("<td colspan=2>").addClass('pvtAxisContainerTD').appendTo(tr2).html(unused);
        
        tr3 = $("<tr>").appendTo(uiTable);
        tr3.append($("<td rowspan=2 style=vertical-align:top>").addClass('pvtAxisContainer pvtRows').attr("valign", "top"));
        $("<td>").addClass('pvtAxisContainer pvtHorizList pvtCols').appendTo(tr3);
        tr4 = $("<tr>").appendTo(uiTable);
        pivotTable = $("<td>").attr("valign", "top").addClass('pvtRendererArea').appendTo(tr4);
        /*if (opts.unusedAttrsVertical === true || unusedAttrsVerticalAutoOverride) {
        ...
        refreshDelayed = (function(_this) {
          return function() {
            ...
            console.log("numInputsToProcess",numInputsToProcess);
            vals = [];
            ...
            $("<button>", {
                type: "button",
                id:"sumAgsAdd"
              }).insertAfter(aggregator).html("Add More").bind("click", function() {
              	sumColCount+=1;
              	insertAttributes().insertAfter(aggregator);
                return false;
              });
            if (numInputsToProcess !== 0) {
              ...
              for (x = t = 0, ref5 = numInputsToProcess; 0 <= ref5 ? t < ref5 : t > ref5; x = 0 <= ref5 ? ++t : --t) {
            	  console.log("insertAttributes");
            	  insertAttributes().insertAfter(aggregator);
              }
                
            	  
            }
            console.log("opts",opts.vals);
            if (initialRender) {
              vals = opts.vals;
              i = 0;
              _this.find(".pvtVals select.pvtAttrDropdown").each(function() {
